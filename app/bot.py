...
DIG_STAR_ACTIONS = {
    ...
    "golden_ticket": ("Золотой билет", "Одна игра 3×3: семь призовых клеток с котоинами или предметами.", 2, "golden_ticket", 1),
...
HELP_SECTIONS = {
    ...
    "moderation": (
        "<b>🛡 Все команды модерации</b>\n"
        "Команды с участником работают ответом на его сообщение или через <code>@ник</code>.\n\n"
        "<b>Нарушения и ограничения</b>\n"
        ...
        "<code>+помощник @ник [срок]</code> · <code>-помощник @ник</code>\n"
        "<code>+модератор @ник [срок]</code> · <code>-модератор @ник</code>\n"
        "<code>+стМодератор @ник [срок]</code> · <code>-стМодератор @ник</code>\n"
        "Срок роли: <code>7д</code>, <code>2 недели</code>, <code>месяц</code>. Назначение — только администрация бота.\n"
        "<code>+админ @ник</code> — дать доступ к админ-профилю Mini App; только владелец бота."
    ),
    "roles": (
        "<b>🔧 Роли персонала</b>\n"
        "Назначение ролей доступно администрации бота. Команду можно отправить ответом или через <code>@ник</code>.\n\n"
        "<code>+помощник @ник [срок]</code> · <code>-помощник @ник</code>\n"
        "<code>+модератор @ник [срок]</code> · <code>-модератор @ник</code>\n"
        "<code>+стМодератор @ник [срок]</code> · <code>-стМодератор @ник</code>\n"
        "Срок: <code>7д</code>, <code>2 недели</code> или <code>месяц</code>.\n\n"
        "<code>+админ @ник</code> — админ-профиль Mini App; только владелец бота.\n"
        "Полный список рабочих команд находится в разделе <b>🛡 Модерация</b>."
    ),
}
    ...
    return json.dumps(
        {
            ...
            "official_alert": state.official_alert,
            "threats": [
                {
...
def is_important_alarm_update(previous: AlertsLocationState, current: AlertsLocationState) -> bool:
    ...
    previous_important = {
        threat.threat_type for threat in previous.threats if threat.threat_type in ALERTS_IMPORTANT_THREATS
    }
    current_important = {
        threat.threat_type for threat in current.threats if threat.threat_type in ALERTS_IMPORTANT_THREATS
    }
    ...
    previous_red = {
        threat.threat_type
        for threat in previous.threats
        if threat.threat_type in ALERTS_IMPORTANT_THREATS and threat.level == "red"
    }
    current_red = {
        threat.threat_type
...
def format_important_alarm_update(
    ...
    if previous.alert_level != "red" and current.alert_level == "red":
        lines.append("Общий уровень повышен до 🔴 красного.")

    previous_types = {threat.threat_type for threat in previous.threats}
    new_important = sorted(
        {
            threat.threat_type
            for threat in current.threats
            if threat.threat_type in ALERTS_IMPORTANT_THREATS
            and (
                threat.threat_type not in previous_types
                or (
                    threat.level == "red"
                    and not any(
                        old.threat_type == threat.threat_type and old.level == "red"
                        for old in previous.threats
                    )
                )
            )
        }
...
async def edit_alarm_status_message(
    bot: Bot,
    chat_id: int,
    ...
    if message_id is not None:
        try:
            ...
            if "message is not modified" in str(exc).casefold():
                return True
            logging.info("Could not edit alarm status message in chat %s: %s", chat_id, exc)
        except (TelegramForbiddenError, TelegramNotFound, TelegramRetryAfter) as exc:
            logging.warning("Could not edit alarm status message in chat %s: %s", chat_id, exc)
...
async def update_alarm_from_api(
    bot: Bot,
    chat_id: int,
    previous: AlertsLocationState,
    current: AlertsLocationState,
...
def alarm_status_text(chat_id: int) -> str:
    ...
    if source in {"neptun", "ukraine_alarm"}:
        ...
        if state is None:
            ...
            lines.append(
                "🟢 Воздушной тревоги и активных угроз нет."
            )
        elif state.official_alert:
            ...
            if labels:
                lines.append(f"Угрозы: <b>{escape(', '.join(labels))}</b>.")
        else:
            labels = list(dict.fromkeys(
                alerts_threat_label(threat.threat_type, "neptun")
                for threat in state.threats
