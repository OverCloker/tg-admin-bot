...
def test_deploy_notifies_all_registered_chats_before_build():
    ...
    assert script.index("resolve_deploy_notice_targets") < script.index("docker compose build bot")
    assert script.index("send_deploy_notice") < script.index('docker compose $COMPOSE_PROFILE_ARGS up -d --no-build --remove-orphans')
